...
if (isset($_POST['minus_submit'])){

	$burger_id = $_POST['trash_id'];
	$burger_name = $_POST['trash_name'];
	$burger_category = $_POST['trash_category'];
	$burger_price = $_POST['trash_price'];

	$burger_bun = $_POST['trash_bun'];
	$burger_patty = $_POST['trash_patty'];
	$burger_dressing = $_POST['trash_dressing'];
	$burger_add_on = $_POST['trash_on_add'];
	$burger_order = $_POST['trash_total'];
	...
	$graph_checking_exist_2 ="SELECT * FROM graph_result_week where name ='$burger_name' and week_1 = '$week' and week_2 = '$weeks'";
		...
		if (mysqli_num_rows($graph_checking_result_2) == 1) {
				while ($graph_checking_row_2 = mysqli_fetch_assoc($graph_checking_result_2)) {
					...
					$graph_query_2 = mysqli_query($connect,$graph_update_total_2);
	...
	$graph_checking_exist_3 ="SELECT * FROM graph_result_month where name ='$burger_name' and month = '$month'";
		...
		if (mysqli_num_rows($graph_checking_result_3) == 1) {
				while ($graph_checking_row_3 = mysqli_fetch_assoc($graph_checking_result_3)) {
					...
					$graph_update_total_2 = "UPDATE graph_result_month SET total = '$graph_sum_total_3' , total_cost = '$graph_sum_cost_3'
	...
	if (mysqli_num_rows($result) >=1){
		...
		$update_sql = "DELETE FROM minute_sales where id = '$burger_id'";
		$result_sql = mysqli_query($connect,$update_sql);
		
		header("location:../casheir/casheir.php?page=6");
		ob_clean();
...
/*
		while ($row = mysqli_fetch_assoc($result)){
			...
			if ($result_update){
				...
				$result_sql = mysqli_query($connect,$update_sql);
				if ($result_sql) {
					header("location:../casheir/casheir.php?page=5");
					ob_clean();
				}
