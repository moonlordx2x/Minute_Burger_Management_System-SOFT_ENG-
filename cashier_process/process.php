...
if (isset($_POST['burger_submit'])){
	...
	if ($stock == '0' || $stock1 == '0' || $stock2 == '0' || $stock3 == '0') {
		...
		ob_clean();
		echo "ERROR INSUFFICIENT PRODUCT";
	}
	else{
	...
	$sql_insert = "INSERT INTO minute_sales(id,name,category,price,total,date,time)
				   values ('$burger_id','$burger_name','$burger_category',
						'$burger_price','$burger_order','$time','$time3')";
	$sql_result = mysqli_query($connect,$sql_insert);

	//daily

	$graph_checking = "SELECT * FROM graph_result where name ='$burger_name' and total = '0' and total_cost = '0'";
	$graph_checking_result = mysqli_query($connect,$graph_checking);
	if (mysqli_num_rows($graph_checking_result) == 1) {

		$graph_update = "SELECT * FROM minute_sales where name ='$burger_name'";
		$graph_result = mysqli_query($connect,$graph_update);
		if (mysqli_num_rows($graph_result) == 1) {
			while ($graph_row = mysqli_fetch_assoc($graph_result)) {
			$price_graph = $graph_row['price'];
			$price_total = $graph_row['total'];
			$graph_total = $price_graph * $price_total;

			$graph_update_1 = "UPDATE graph_result SET total = '$price_total' , total_cost = '$graph_total'
				where name ='$burger_name' and date_1 = '$time'";
			$graph_query = mysqli_query($connect,$graph_update_1);

			if ($graph_query){
					echo "Daily set C 1<br>";
			}	
		...
		if (mysqli_num_rows($graph_checking_result) == 1) {
				while ($graph_checking_row = mysqli_fetch_assoc($graph_checking_result)) {
					...
					$graph_sum_total = $graph_total_1 + $burger_order;
					$sum = $burger_price * $burger_order;
					...
					where name ='$burger_name' and date_1 = '$time'";
					$graph_query_1y = mysqli_query($connect,$graph_update_total);


					if ($graph_query_1y){
					echo "Daily set C 2<br>";
					}
	...
	$graph_checking_2 = "SELECT * FROM graph_result_week where name ='$burger_name' and total = '0' and total_cost = '0'";
	$graph_checking_result_2 = mysqli_query($connect,$graph_checking_2);
	if (mysqli_num_rows($graph_checking_result_2) == 1) {

		$graph_update_2 = "SELECT * FROM minute_sales where name ='$burger_name'";
		$graph_result_2 = mysqli_query($connect,$graph_update_2);
		if (mysqli_num_rows($graph_result_2) == 1) {
			while ($graph_row_2 = mysqli_fetch_assoc($graph_result_2)) {
			$price_graph_2 = $graph_row_2['price'];
			$price_total_2 = $graph_row_2['total'];
			$graph_total_2 = $price_graph_2 * $price_total_2;

			$graph_update_2 = "UPDATE graph_result_week SET total = '$price_total_2' , total_cost = '$graph_total_2'
				where name ='$burger_name' and week_1 = '$week' and week_2 = '$weeks'";
			$graph_query_2v = mysqli_query($connect,$graph_update_2);

			if ($graph_query_2v){
				echo "Week set B 1<br>";
			}		
		...
		if (mysqli_num_rows($graph_checking_result_2) == 1) {
				while ($graph_checking_row_2 = mysqli_fetch_assoc($graph_checking_result_2)) {
					...
					$graph_sum_total_2 = $graph_total_2 + $burger_order;
					$sum_2 = $burger_price * $burger_order;
					$graph_sum_cost_2 = $graph_total_cost_2 + $sum_2;

					$graph_update_total_2 = "UPDATE graph_result_week SET total = '$graph_sum_total_2' , total_cost = '$graph_sum_cost_2'
					where name ='$burger_name' and week_1 = '$week' and week_2 = '$weeks'";
					$graph_query_2y = mysqli_query($connect,$graph_update_total_2);

					if ($graph_query_2y){
					echo "Week set B 2<br>";
					}	
	...
	$graph_checking_3 = "SELECT * FROM graph_result_month where name ='$burger_name' and total = '0' and total_cost = '0'";
	$graph_checking_result_3 = mysqli_query($connect,$graph_checking_3);
	if (mysqli_num_rows($graph_checking_result_3) == 1) {

		$graph_update_3 = "SELECT * FROM minute_sales where name ='$burger_name'";
		$graph_result_3 = mysqli_query($connect,$graph_update_3);
		if (mysqli_num_rows($graph_result_3) == 1){
			while ($graph_row_3 = mysqli_fetch_assoc($graph_result_3)) {
			$price_graph_3 = $graph_row_3['price'];
			$price_total_3 = $graph_row_3['total'];
			$graph_total_3 = $price_graph_3 * $price_total_3;

			$graph_update_v3 = "UPDATE graph_result_month SET total = '$price_total_3' , total_cost = '$graph_total_3'
				where name ='$burger_name' and month_name = '$month'";
			$graph_query_3v = mysqli_query($connect,$graph_update_v3);

			if ($graph_query_3v){
					echo "Month set A 1<br>";
			}	
		...
		$graph_checking_exist_3 = "SELECT * FROM graph_result_month where name ='$burger_name' and month_name = '$month'";
		...
		$graph_total_cost_3 = $graph_checking_row_3['total_cost'];
					...
					$graph_sum_total_3 = $graph_total_3 + $burger_order;
					$sum_3 = $burger_price * $burger_order;
					$graph_sum_cost_3 = $graph_total_cost_3 + $sum_3;

					$graph_update_total_3 = "UPDATE graph_result_month SET total = '$graph_sum_total_3' , total_cost = '$graph_sum_cost_3'
					where name ='$burger_name' and month_name = '$month'";
					$graph_query_3y = mysqli_query($connect,$graph_update_total_3);

					if ($graph_query_3y) {
					echo "Month set A 2<br>";
				}	
...
/*
		...
		while ($row = mysqli_fetch_assoc($result)){
			...
			$update = "UPDATE minute_stock set stock_quantity IN ('$order1','$order2','$order3','$order4') where stock_id = '$id_stock'";
